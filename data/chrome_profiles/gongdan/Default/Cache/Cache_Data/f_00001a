...
const krds_modal = {
  ...
  close: (event) => {
		...
		this.modalCloseTriggers.forEach(e => {
			...
			e.addEventListener('keydown', ele => { //닫기버튼에서 탭 키 누르면 모달 내 첫번쨰 포커스로 키보드 이동
				//모달 닫기 이벤트 추가 최진곤
				if (e.classList.contains('btn-close') || e.classList.contains('btn-back')) {
				//모달 닫기 이벤트 추가 end
  ...
  open: () => {
		...
		this.modalOpenTriggers.forEach(e => {
			e.addEventListener('click', ele => {
				...
				const param = e.getAttribute('data-param');

				if (id) {
				  e.setAttribute("data-modal-id", id);
		          e.classList.add("modal-opened");
		          e.setAttribute("tabindex", "-1");
		          krds_modal.openModalCustom(id, param);
				}
			});
		});
  },
  openModalCustom(id,param , event) {
  	if(id.indexOf("LAYER_POPUP_") == 0){
	    
		const $modalCheck = document.querySelectorAll("#"+id+"_DIV").length;
	    let $modal_insert = modal(id,param);
		if (!document.getElementById(id+"_DIV")) {  // 요소가 없다면, 
			body.append(modalService._fromHTML(modalService._makeModalDivTemplate(id+"_DIV")));		
		}
	    $modalCheck < 1 ? $("#"+id+"_DIV").append($modal_insert) : $modal_insert = '';
		this.modalCloseTrigger = document.querySelectorAll('.close-modal');
		
		krds_modal.close(event);
		...
		const modalConts = modalElement.querySelector(".modal-conts");
		    ...
		    dialogElement.addEventListener(
		      ...
		      (event) => {
		        if (event.key === "Escape" || event.key === "Esc") {
		          krds_modal.closeModal(dialogElement.closest(".krds-modal").id);
		        }
		      },
		      { once: true }
		    );
		    ...
		    krds_modal.updateZIndex(modalElement);
			...
			//, 25.09.16_popup.js 에 등록된 팝업이 아니면 krds 기본 제공 openModal 함수를 탄다.
			if($modal_insert == ""){
				...
				$modalCheck < 1 ? $modal_insert : $("#"+id+"_DIV").empty();
				if(param){
					$modal_insert=$modal_insert+param;
				}
			
				$("#"+id+"_DIV").load($modal_insert
				,function(){
					this.modalCloseTrigger = document.querySelectorAll('.close-modal');
					...
					if(modalElement){
						...
						if (!krds_modal.outsideClickHandlers[id]) {
						  krds_modal.outsideClickHandlers[id] = (event) => {
							if (!event.target.closest(".modal-content")) {
							  // modalTitle.focus();
							  focusables[0].focus();
						...
						// 이벤트 리스너 제거 후 다시 등록
						modalElement.removeEventListener("click", krds_modal.outsideClickHandlers[id]);
						modalElement.addEventListener("click", krds_modal.outsideClickHandlers[id]);
					...
					let txt = '';
					txt+='<section id="'+id+'" class="krds-modal" role="dialog" aria-labelledby="tit-modal-error" style="width:520px;">';
        			txt+='<div class="modal-dialog">';
            		txt+='<div class="modal-content">';
                	txt+='<div class="modal-conts">';
                  	txt+='<div class="error-wrap">';
                  	txt+='<h1 id="tit-modal-error" class="error-h4">홈페이지 이용에 불편을 드려 죄송합니다.</h1>';
                  	txt+='<a href="/npbs/index.jsp" class="krds-btn primary medium btn-main">메인으로</a>';
                  	txt+='</div>';
                	txt+='</div>';
                	txt+='<button type="button" class="krds-btn medium icon btn-close close-modal">';
                    txt+='<span class="sr-only">닫기</span>';
                    txt+='<i class="svg-icon ico-popup-close"></i>';
                	txt+='</button>';
            		txt+='</div>';
        			txt+='</div>';
        			txt+='<div class="modal-back in"></div>';
    				txt+='</section>';
					$("#"+id+"_DIV").html(txt);
  ...
  closeModal(id ,event) {
    const modalElement = document.getElementById(id);
    const openModals = document.querySelectorAll(".krds-modal.in:not(.sample)");
    const modalBack = modalElement.querySelector(".modal-back");
    ...
    // 모달을 열었던 버튼으로 포커스 복귀
    this.returnFocusToTrigger(id , event);
  },
